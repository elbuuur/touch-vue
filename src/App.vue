  ...
  methods: {
    addTicker() {
      ...
      setInterval(async () => {
        ...
        let tickerPrice = await (await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${currentTicker.name}&tsyms=USD&api_key=${apiKey}`)).json()

        this.tickers.find(tickersItem => tickersItem.name === currentTicker.name).price = tickerPrice.USD

        if(currentTicker.name === this.selectedTicker?.name) {
